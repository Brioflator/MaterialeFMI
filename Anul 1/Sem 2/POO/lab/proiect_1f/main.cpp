...
public:
    ...
    {
       cout<<"Putem sa avem o singura editura partenera, iar aceasta este: "<<edy.get_editure()<<endl;
    }

    string get_edy() {return this->edy.get_editure();}
    void set_edy (string a) {this->edy.set_editure(a);}
};
ST* ST::obj=0;
...
ostream& carte::afis(ostream& out)const
{
...
istream& carte::citi(istream& in)
{
    ...
    if (this->titlu!=NULL)
        delete[]this->titlu;
    this->titlu=new char[strlen(titlu_aux)+1];
    strcpy(this->titlu, titlu_aux);
    cout<<"Autor: ";
    in>>this->autor;
    cout<<"Pret: ";
    in>>this->pret;
    ...
    this->total_carti=this->total_carti+this->numar_carti;
...
class revista: public virtual carte
{
protected:
    string lucios;
public:
    ...
    revista(char* titlu, int len_titlu, float pret, int numar_carti, string autor, string lucios): carte(titlu, len_titlu, pret, numar_carti, autor)
    {
        this->lucios=lucios;
    }
    //copy constructor
    revista(const revista& r): carte(r)
    {
        this->lucios=r.lucios;
    }
    ...
    {
        carte::citi(in);
        cout<<"Pagini lucioase: ";
        in>>this->lucios;

        return in;
    }
    friend istream& operator >> (istream& in, revista& r)
    {
        return r.citi(in);
    }
    ...
    friend ostream& operator << (ostream& out, const revista& r)
    {
        return r.afis(out);
    }
};

class copii: public virtual carte
{
protected:
    string desene;
...
public:
    ...
    //constructor cu toti parametrii
    copii(char* titlu, int len_titlu, float pret, int numar_carti, string autor, string desene): carte(titlu, len_titlu, pret, numar_carti, autor)
    {
        this->desene=desene;
    }
    //copy constructor
    copii(const copii& c): carte(c)
    {
        this->desene=c.desene;
    }
    ...
    friend ostream& operator << (ostream& out, const copii& c)
    {
        return c.afis(out);
    }
};

class banda: public virtual revista, public virtual copii
{
protected:
    string colorat;
public:
    //constructor fara parametrii
    banda():revista(),copii(),carte()
    {
        desene="nedeterminat";
    }
    //constructor cu toti parametrii
    banda(char* titlu, int len_titlu, float pret, int numar_carti, string autor, string colorat):revista(titlu, len_titlu, pret, numar_carti, autor, lucios), copii(titlu, len_titlu, pret, numar_carti, autor,desene), carte(titlu, len_titlu, pret, numar_carti, autor)
    {
        this->colorat=colorat;
    }
    //copy constructor
    banda(const banda& b): revista(b), copii(b), carte(b)
    {
        this->colorat=b.colorat;
    }

    //operator egal
    banda operator=(const banda& b)
    {
        if(this!=&b)
        {
            carte::operator=(b);
    ...
    virtual istream& citi(istream& in)
    {
        ...
        cout<<"Desene: ";
        in>>this->desene;
        cout<<"Pagini colorate: ";
        in>>this->colorat;

        return in;
    }
    friend istream& operator >> (istream& in, banda& b)
    {
        return b.citi(in);
    }
    ...
    virtual ostream& afis(ostream& out)const
    {
        ...
        cout<<endl<<"Desene: ";
        out<<this->desene;
        cout<<endl<<"Pagini colorate: ";
        out<<this->colorat;
        cout<<endl<<" \n"<<endl;

        return out;
    }
    friend ostream& operator << (ostream& out, const banda& b)
    {
        return b.afis(out);
    }
};
...
public:
                ...
                if (p=="1")
                    {
                        ...
                        total=suma(total,c.get_pret());
                        autori=suma(autori, c.get_autor());
                    }
    ...
    string get_autori(){return autori;}
...
public:
    ...
    client operator+=(const carte& a)
    {
        this->cosul.push_back(a);
        return *this;
    }

    client operator*=(const carte& a)
    {
        this->bonus.push_back(a);
        return *this;
    }
...
int main()
{
    ...
    ST *e=e->get_insta();
    e->set_edy("Humanitas");
    e->get_insta()->afisare_editura();
    //cout<<"Putem sa avem o singura editura partenera, iar aceasta este: "<<e->get_edy()<<endl;
                    ...
                    while(true)
                        {
                            ...
                            if(pl==3)
                            {
                                ...
                                {int k=vanzator.librarie[i].get_cantitate();
                                vanzator.librarie[i].set_cantitate(0);
                                    k+=10;
                                   vanzator.librarie[i].set_cantitate(k);}
                            }
                        }
                    cout<<endl<<"Revista de azi: "<<endl;
                    cin>>rev;

                    cout<<endl<<"Cartea de copii pe care trebuie sa o promovam: "<<endl;
                    cin>>poveste;

                    cout<<endl<<"Banda desenata cadou: "<<endl;
                    cin>>BD;

                }
        }

    cout<<vanzator.get_autori()<<endl;
    cout<<vanzator.get_total()<<endl;
                                    ...
                                    if (strcmp(m, vanzator.librarie[i].get_titlu())==0)
                                        {
                                            ...
                                            cumparator+=vanzator.librarie[i];                   // indexare in meniu cu operatorii destinati
                                        }
                                }
    ...
    cout<<"Vreti si o revista?"<<endl;
    cout<<"DA/NU"<<endl;
    string input_4;
    cin>>input_4;

    cout<<"V ar interesa cartea de copii "<<poveste.get_titlu()<<"?"<<endl;
    cout<<"DA/NU"<<endl;

    string input_5;
    cin>>input_5;

    cout<<"Din partea noastra primiti si o banda desenata cadou!"<<endl;
    ...
    if(input_5=="DA"||input_5=="da"||input_5=="Da")
    {
        bani=bani-cumparator.get_total()-poveste.get_pret();
        if(bani<0){cout<<"Nu va permiteti cosul, la revedere!"; return 0;}
            cout<<"Totalul comenzii este de "<<cumparator.get_total()+poveste.get_pret()<<"lei si mai aveti la dizpozitie ";
    }
    else
    {
    ...
    if(input_4=="DA"||input_4=="da"||input_4=="Da") cumparator*=rev;
    if(input_5=="DA"||input_5=="da"||input_5=="Da") cumparator*=poveste;
    cumparator*=BD;
    carte D;
    D.set_titlu("contact");
    D.set_autor("0742153968");
    D.set_pret(0);
    cumparator*=D;
    afis_bonus(cumparator.bonus);
    cout<<BD;
