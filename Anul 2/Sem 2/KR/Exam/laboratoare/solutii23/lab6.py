...
class InfoJoc:
    ...
    def final(self):
        rez = elem_identice(self.matr[0]) or elem_identice(self.matr[1]) or elem_identice(self.matr[2]) \
              or elem_identice([self.matr[i][0] for i in range(0, 3)])\
              or elem_identice([self.matr[i][1]for i in range(0, 3)])\
              or elem_identice([self.matr[i][2]for i in range(0, 3)])\
              or elem_identice([self.matr[i][i] for i in range(0, 3)])\
              or elem_identice([self.matr[i][2-i] for i in range(0, 3)])
        if rez:
            return rez
        ...
        for i in range(InfoJoc.NR_COLOANE):
            for j in range(InfoJoc.NR_COLOANE):
                if self.matr[i][j] == InfoJoc.GOL:
                    copieMatr = copy.deepcopy(self.matr)
                    copieMatr[i][j] = jucator
                    lista.append(InfoJoc(copieMatr))
        return lista
    ...
    def linii_deschise(self, jucator):
        rez = self.linie_deschisa(self.matr[0], jucator) + self.linie_deschisa(self.matr[1], jucator) + self.linie_deschisa(self.matr[2], jucator) \
              + self.linie_deschisa([self.matr[i][0] for i in range(0, 3)], jucator)\
              + self.linie_deschisa([self.matr[i][1] for i in range(0, 3)], jucator)\
              + self.linie_deschisa([self.matr[i][2] for i in range(0, 3)], jucator)\
              + self.linie_deschisa([self.matr[i][i] for i in range(0, 3)], jucator)\
              + self.linie_deschisa([self.matr[i][2-i] for i in range(0, 3)], jucator)
        return rez

    #restAdancime = cat mai are pana ajunge la adancimea maxima
    def estimeaza_scor(self, restAdancime):
        rez_final = self.final()
        if rez_final == InfoJoc.JMAX:
            return 1000000+restAdancime
        elif rez_final == InfoJoc.JMIN:
            return -100000-restAdancime
        elif rez_final == "remiza":
            return 0
...
def min_max(stare):
    ...
    if InfoJoc.final(stare.tabla_joc) or stare.adancime >= 9:
        return stare.tabla_joc.estimeaza_scor(stare, stare.adancime)
    else:
        listaSucc = stare.mutari()
        listaEstimari = [(s.tabla_joc.estimeaza_scor(s), s) for s in listaSucc]

        if stare.j_curent == InfoJoc.JMIN:
            (est, stare.stare_aleasa) = max(listaEstimari)
        else:
            (est, stare.stare_aleasa) = min(listaEstimari)

        min_max(stare.stare_aleasa)


def alpha_beta(alpha, beta, stare):
    if InfoJoc.final(stare) or stare.adancime >= 9:
        return InfoJoc.estimeaza_scor(stare, stare.adancime)
    listaSucc = stare.mutari()
