...
class Graf:
    ...
    def succesori(self, nod):
        ...
        for istiva, stiva in enumerate(nod.info):
            ...
            for istiva2, stiva2 in enumerate(copieStive):
                ...
                nodNou=NodArbore(stiveNoi,nod,nod.g+ord(bloc) - ord('a'),self.calculeaza_h(stiveNoi))
                if not nodNou.vizitat():
                    l.append(nodNou)
        return l

    def calculeaza_h(self, info, euristica = 'banala'):
        if euristica == 'banala':
            if self.scop(info):
                return 0
            else:
                return min([ord(stiva[len(stiva) - 1]) - ord('a') if stiva else 1e9 for stiva in info])
        elif euristica == 'euristica mutari':
            if self.scop(info):
                return 0
            else:
                return min([sum([abs(len(info[i]) - len(stiva)) for stiva in scop]) for i, scop in enumerate(scopuri)])
    ...
    def valideaza(self):
        ...
        lenStart =  sum([len(stiva) for stiva in self.start])
        cond2 = all( [lenStart == sum([len(stiva) for stiva in scop]) for scop in self.scopuri])
...
# [print(nod.info) for nod in gr.succesori(NodArbore(start))]
# print(gr.scop([['b', 'c'], [], ['d', 'a']]))
# print(start)
# print(scopuri)