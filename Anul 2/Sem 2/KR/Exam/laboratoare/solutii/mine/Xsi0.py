...
def min_max(stare):
    ...
    mutariExp = [min_max(mutare) for mutare in stare.mutari_posibile]
    if stare.j_curent == InfoJoc.JMAX:
        stare.stare_aleasa = max(mutariExp, key=lambda x: x.estimare)
    else:
        stare.stare_aleasa = min(mutariExp, key=lambda x: x.estimare)
