...
class Graf:
    ...
    def valideaza(self):
        ...
        if any([len(linie) != 3 for linie in self.start]):
            return False
        matrExt = sum(self.start, start=[])
        if sorted(matrExt) != list(range(9)):
            return False
        nrInv=0
        for i1, elem1 in enumerate(matrExt):
            for i2, elem2 in enumerate(matrExt[i1+1:]):
    ...
    def succesori(self, nod):
        ...
        for lg in range(3):
            ...
            if ok:
                break
        diretii=[[-1, 0], [1, 0], [0, -1], [0, 1]]
        for d in diretii:
            lP = lg+d[0]
            cP = cg+d[1]
            if 0 <= lP <= 2 and 0 <= cP <= 2:
                stareNoua = copy.deepcopy(nod.info)
                stareNoua[lP][cP], stareNoua[lg][cg] = stareNoua[lg][cg], stareNoua[lP][cP]
...
f=open("inpuut.txt", "r")
start = [[int(x) for x in lin.strip().split()] for lin in f.readlines()]
