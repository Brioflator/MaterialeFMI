...
void help()
{
    ...
    printf("hist: prints the command history of the current session \n");
...
void execute_command(char **words, int nr_args)
{
...
int main()
{
    ...
    while (1)
    {
        ...
        // parser
        //  p va contine fiecare cuvant al comenzii
        while (p != NULL)
        {
            ...
            if (!strcmp(c, "|"))
            {
                ...
                if (nr_words > 0)
                {
                    free(output);
                    output = malloc(NSize * sizeof(char));
                    execute_command(words, nr_words);
                }
